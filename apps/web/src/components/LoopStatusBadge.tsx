...
export function LoopStatusBadge({ projectId }: LoopStatusBadgeProps) {
  ...
  const stateColor = {
    ...
    STOPPING: 'bg-emerald-500 animate-pulse',
    STOPPED: 'bg-zinc-400',
    ...
    <div className="flex items-center gap-2 px-2 py-1 rounded-md bg-zinc-200/50 dark:bg-zinc-700/50">
      ...
      {isRunning ? (
        ...
        <button
          onClick={handleStart}
          disabled={isLoading}
          className="p-1 text-zinc-500 dark:text-zinc-400 hover:text-emerald-500 dark:hover:text-emerald-400 disabled:opacity-50"
          title="Start Loop"
        >
